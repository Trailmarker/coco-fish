...
function setup() {
    ...
    Array.from("COCONUTSTUDIOS").map((c, i) => {
        const flock = new Flock();
        
        for (var j = 0; j < 5; i++) {
            let b = new Boid(width / 2, height / 2, images[c], 150, 150, finalPositions[i]);
            flock.addBoid(b);
        }
...
function Boid(x, y, img, width, height, finalPosition) {
    this.img = img;
    this.width = width;
    this.height = height;
...
Boid.prototype.flock = function (boids) {
    ...
    // let trav = this.travel();      // Destination
...
Boid.prototype.travel = function () {
    let dest = createVector(this.finalPosition[0], this.finalPosition[1]);
    return this.seek(dest); 
}
